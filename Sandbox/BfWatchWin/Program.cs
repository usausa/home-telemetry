using System.Diagnostics;
using System.Runtime.InteropServices;
using System.Transactions;

using Windows.Devices.Bluetooth;
using Windows.Devices.Bluetooth.GenericAttributeProfile;
using Windows.Devices.Enumeration;
using Windows.Security.Cryptography;
using Windows.Storage.Streams;

// Const
var serviceUuid = Guid.Parse("6e400001-b5a3-f393-e0a9-e50e24dcca9e");
var txUuid = Guid.Parse("6e400002-b5a3-f393-e0a9-e50e24dcca9e");
var rxUuid = Guid.Parse("6e400003-b5a3-f393-e0a9-e50e24dcca9e");

// Parameter
var targetAddress = Convert.ToInt64(args[0], 16);

// Data
var devices = new Dictionary<long, DeviceInformation>();

// Watcher
static bool TryGetAddress(IReadOnlyDictionary<string, object> properties, out long address)
{
    if (properties.TryGetValue("System.Devices.Aep.DeviceAddress", out var value))
    {
        try
        {
            address = Convert.ToInt64(value.ToString()?.Replace(":", string.Empty, StringComparison.OrdinalIgnoreCase), 16);
            return true;
        }
        catch (FormatException)
        {
            // Ignore
        }
    }

    address = default;
    return false;
}

var watcher =
    DeviceInformation.CreateWatcher(
        "(System.Devices.Aep.ProtocolId:=\"{bb7bb05e-5972-42b5-94fc-76eaa7084d49}\")",
        new[] { "System.Devices.Aep.DeviceAddress" },
        DeviceInformationKind.AssociationEndpoint);

watcher.Added += (_, information) =>
{
    if (TryGetAddress(information.Properties, out var address))
    {
        lock (devices)
        {
            devices[address] = information;
        }
        Debug.WriteLine($"Device Added: {address:X12} {information.Name}");
    }
};
watcher.Removed += (_, information) =>
{
    if (TryGetAddress(information.Properties, out var address))
    {
        lock (devices)
        {
            devices.Remove(address);
        }
        Debug.WriteLine($"Device Removed: {address:X12}");
    }
};

watcher.Start();

while (true)
{
    await Task.Delay(1000).ConfigureAwait(false);

    var device = default(DeviceInformation);
    lock (devices)
    {
        if (!devices.TryGetValue(targetAddress, out device))
        {
            continue;
        }
    }

#pragma warning disable CA1031
    try
    {
        var bleDevice = await BluetoothLEDevice.FromIdAsync(device.Id);
        if (bleDevice is null)
        {
            Debug.WriteLine("Find id failed.");
            continue;
        }

        var result = await bleDevice.GetGattServicesAsync(BluetoothCacheMode.Uncached);
        if (result.Status != GattCommunicationStatus.Success)
        {
            Debug.WriteLine($"Get gatt services failed. result=[{result.Status}]");
            continue;
        }

        var service = result.Services.FirstOrDefault(x => x.Uuid == serviceUuid);
        if (service is null)
        {
            Debug.WriteLine("Services is not found.");
            continue;
        }

        var accessStatus = await service.RequestAccessAsync();
        if (accessStatus != DeviceAccessStatus.Allowed)
        {
            Debug.WriteLine($"Request access failed. result=[{accessStatus}]");
            continue;
        }

        var characteristicsResult = await service.GetCharacteristicsAsync(BluetoothCacheMode.Uncached);
        if (characteristicsResult.Status != GattCommunicationStatus.Success)
        {
            Debug.WriteLine($"Get characteristics failed. result=[{characteristicsResult.Status}]");
            continue;
        }

        var tx = characteristicsResult.Characteristics.FirstOrDefault(x => x.Uuid == txUuid);
        var rx = characteristicsResult.Characteristics.FirstOrDefault(x => x.Uuid == rxUuid);
        if ((tx is null) || (rx is null))
        {
            Debug.WriteLine("Characteristics is not found.");
            continue;
        }

        using var writer = new DataWriter();
        writer.WriteBytes(MakeCommand([0x08]));
        var txResult = await tx.WriteValueWithResultAsync(writer.DetachBuffer());
        if (txResult.Status != GattCommunicationStatus.Success)
        {
            Debug.WriteLine($"Write value failed. result=[{txResult.Status}]");
            continue;
        }

        var rxResult = await rx.ReadValueAsync(BluetoothCacheMode.Uncached);
        if (rxResult.Status != GattCommunicationStatus.Success)
        {
            Debug.WriteLine($"Read value failed. result=[{rxResult.Status}]");
            continue;
        }

        //var format = rx.PresentationFormats[0];
        CryptographicBuffer.CopyToByteArray(rxResult.Value, out var data);
        Debug.WriteLine(data.Length);
    }
    catch (COMException e)
    {
        Debug.WriteLine(e);
    }
    catch (Exception e)
    {
        Debug.WriteLine(e);
    }
#pragma warning restore CA1031
}

static byte[] MakeCommand(byte[] payload)
{
    var array = new byte[payload.Length + 4];
    array[0] = 0xAA;
    array[1] = (byte)((uint)(payload.Length >> 8) & 0xFF);
    array[2] = (byte)((uint)payload.Length & 0xFFu);
    payload.CopyTo(array, 3);
    array[^1] = CalcCrc8(payload);
    return array;
}

static byte CalcCrc8(byte[] payload)
{
    var array = new[]
    {
        
    };
    var num = 0;
    foreach (var b in payload)
    {
        num = array[(num ^ b) & 0xFF];
    }
    return (byte)num;
}